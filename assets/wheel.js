...
function buildWheel(){// Use conic-gradient for segments
 ...
 genres.forEach((g,i)=>{const label=document.createElement('div');label.className='segment-label';label.textContent=g;const angle=(i*step)+(step/2);label.style.transform=`rotate(${angle}deg) translate(0,-50%)`;wheel.appendChild(label);});
}

function spin(){if(spinning)return;spinning=true;selectedIndex=null;startQuizBtn.classList.add('hidden');selectedGenreEl.textContent='Spinning...';
 const step=360/genres.length;const targetIndex=Math.floor(Math.random()*genres.length);selectedIndex=targetIndex;const targetAngle=(360-(targetIndex*step+step/2));const extraTurns=360*5;const finalAngle=extraTurns+targetAngle;wheel.style.transition='transform 6s cubic-bezier(.17,.67,.32,1.25)';wheel.style.transform=`rotate(${finalAngle}deg)`;setTimeout(()=>{const genre=genres[selectedIndex];selectedGenreEl.textContent=`Genre: ${genre}`;startQuizBtn.classList.remove('hidden');localStorage.setItem('selectedGenre',genre);spinning=false;},6000);}
